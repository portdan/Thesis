 ...
 *
 */
public class StateActionStateSASPreprocessor {

	private static final Logger LOGGER = Logger.getLogger(StateActionStateSASPreprocessor.class);
	...
	//dictionaries
	public Map<String,Integer> varCodes = new HashMap<String, Integer>();
	public Map<String,Integer> valCodes = new HashMap<String, Integer>();
	 ...
	 * @param addl
	 */
	public StateActionStateSASPreprocessor(SASDomain sas){
		this(sas,new MapConfiguration());
	}
	 ...
	 * @param config
	 */
	public StateActionStateSASPreprocessor(SASDomain sas, ConfigurationInterface config){
		this.sas = sas;
		...
		for (SASOperator action : sas.operators) {
			agentOperators.get(ALL).add(action);
		}
	 ...
	 * @return
	 */
	public Problem getProblemForAgent(){
		Domain d = agentDomains.get(ALL);
		...
		for(SASOperator op : agentOperators.get(ALL)){
			...
			Action a = new Action(op.name,op.label, ALL, pre, eff, op.isPublic);
			LOGGER.info(a);
			if(!unitCost){
				a.setCost(op.cost);
			}
		...
		return new Problem(ALL, initState, goalSuperState, actions,allActions,publicActions);
	}
