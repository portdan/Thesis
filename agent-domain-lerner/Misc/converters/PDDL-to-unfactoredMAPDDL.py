      ...
      for action in self.ground_actions:
        if self.parameters.is_typed:
          ...
          for obj in problem.get_objects_of_type(param[1]):
            new_objects = { k : v for k, v in action.objects.items() }
...
class PlanningProblem(object):
  ...
  def __init__(self, domainfile, problemfile,agentfile):
    ...
    for predicate in self.init:
      ...
      if gp in self.ground_facts:
        gp =  list(filter(lambda f: f==gp,self.ground_facts))[0]
        #print gp
  ...
  def find_public_facts(self):
    for ag1 in self.agents.items():
      for ag2 in self.agents.items():
        if ag1[1].name != ag2[1].name:
          for a1 in ag1[1].ground_actions:
            #print "a1: " + str(a1)
            for a2 in ag2[1].ground_actions:
              #print "a2: " + str(a1)
  ...
  def print_problem(self):
    ...
    for a in self.agent_predicates.keys():
      print (str(a) + ":")
  ...
  def write_unfactored_problem(self, output_file):
    ...
    for obj in self.agents[self.agent_list[0]].objects:
      ...
      if not t in self.constants.keys() or not obj in self.constants[t]:
        to_write += "\t" + obj + " - " + t + "\n"
...
def main():
    ...
    pp.write_unfactored_domain(args.output + "/" + args.domain + ".pddl")
    pp.write_unfactored_problem(args.output + "/" + args.problem + ".pddl")
...
def parse_args():
    ...
    argparser.add_argument(
        "folder", help="path to folder", type=str)
    argparser.add_argument(
